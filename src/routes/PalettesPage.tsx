...
export default function PalettesPage() {
  ...
  const category = index?.categories.find((c) => c.kind === "palettes");
    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24">
          <Palette size={64} className="text-shell-text-primary opacity-20 mb-4" />
          <p className="text-shell-text-secondary">No palettes items yet</p>
        </div>
      ) : (
