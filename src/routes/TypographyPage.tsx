...
export default function TypographyPage() {
  ...
  const category = index?.categories.find((c) => c.kind === "typography");
    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24">
          <Type size={64} className="text-shell-text-primary opacity-20 mb-4" />
          <p className="text-shell-text-secondary">No typography items yet</p>
        </div>
      ) : (
