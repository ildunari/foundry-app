...
export default function PatternsPage() {
  ...
  const category = index?.categories.find((c) => c.kind === "patterns");
    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24">
          <Grid3x3 size={64} className="text-shell-text-primary opacity-20 mb-4" />
          <p className="text-shell-text-secondary">No patterns items yet</p>
        </div>
      ) : (
