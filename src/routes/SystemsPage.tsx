...
export default function SystemsPage() {
  ...
  const category = index?.categories.find((c) => c.kind === "systems");
    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24">
          <Building2 size={64} className="text-shell-text-primary opacity-20 mb-4" />
          <p className="text-shell-text-secondary">No systems items yet</p>
        </div>
      ) : (
