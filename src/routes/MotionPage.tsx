...
export default function MotionPage() {
  ...
  const category = index?.categories.find((c) => c.kind === "motion");
    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24">
          <Zap size={64} className="text-shell-text-primary opacity-20 mb-4" />
          <p className="text-shell-text-secondary">No motion items yet</p>
        </div>
      ) : (
